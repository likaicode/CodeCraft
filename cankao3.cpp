...
public:
    ...
    void topoSort(vector<int> &degs,bool doSoring){
        ...
        while(!que.empty()){
            ...
            que.pop();
            for(int &v:G[u]) {
                if(0==--degs[v])  //遍历入度为0的节点的邻接表并删除入度为1的节点的邻接表
                    que.push(v);
            }
        }
        int cnt=0;
    ...
    void constructP2(){
        ...
        for(int i=0;i<nodeCnt;i++){
            auto &gi=G[i];
            for(int &k:gi){//对i的所有邻接点k进行遍历 
                auto &gk=G[k];
                for(int &j:gk){//对k的所有邻接点j进行遍历
                    if(j!=i){
                        P2inv[j][i].push_back(k);//节点i能够通过节点k访问到节点j
                    }//P2inv[j]为一个unordered_map
                }//P2inv[j][i]为一个vector 存放节点i能够经过一个节点到达节点j的那个节点
            }
